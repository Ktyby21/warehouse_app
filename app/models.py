...
class Worker(db.Model):
    ...
    # Обновленные связи с backref, чтобы использовать их только в Worker
...
class UserAccount(db.Model, UserMixin):
    ...
    # Добавляем связь для доступа к объекту уровня доступа
    access_level = db.relationship('UserAccessLevel')

    # Relationship to link back to the Worker model
    worker = db.relationship('Worker', backref='user_account', uselist=False)

    # Переопределение метода get_id для Flask-Login
    def get_id(self):
...
class Message(db.Model):
    ...
    # Определение связи с отправителем
    sender = db.relationship('Worker', backref='sent_messages')

    # Настройка связи с получателями
...
class MessageRecipient(db.Model):
    ...
    # Определение связей для ForeignKey
